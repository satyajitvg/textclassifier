# -*- coding: utf-8 -*-
"""
Bernoulli Naive Bayes Classifer for Text
Based on http://nlp.stanford.edu/IR-book/html/htmledition/the-bernoulli-model-1.html

Feature Selection done using Information Gain
...
class NBTrain:
    ...
    def addDocument(self, tokens, class_label):
        ...
        for token in tokens:
            self.token_df_counts[token][class_label]+=1
    ...
    def train(self):
        ...
        for token, class_counts in self.token_df_counts.items():
        ...
        for token in selected_tokens.keys():
            for class_label in self.class_label_counts.keys():
                self.condprobs[token][class_label] = (token_df_counts[token][class_label] + 1)/(self.class_label_counts[class_label] +2)
